...
const alg = "RandomGrower_7non3nor"
const rcnt = 10
var cell_heat:={}
var pass_heat:={}
...
func _ready() -> void:
	...
	for _j in range(5000):
		...
		line="%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%s;%f;%d;%d"%[Seed,$FloorArchitect.minimum_room_count,nodes["Leaves"].size(),nodes["Connectors"].size(),nodes["3Cross"].size(),nodes["4Cross"].size(),bap[0]["ArticulationPoints"].size(),bap[0]["Bridges"].size(),diam,layout_e_t-layout_b_t,dist_e_t-dist_b_t,bap_e_t-bap_b_t,nodes_e_t-nodes_b_t,alg,admp,mmdmp,mpl]
		output.store_line(line)
